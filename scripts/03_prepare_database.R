...
############################################################################-
# 2. Create and label variables ----
############################################################################-

db$exp <- db$age-db$esc-6
db <- db %>% mutate(exp = ifelse(exp<0, 0, exp))
...
db <- db %>%mutate(sector=case_when(rama2d<10 ~1, 
               ...
               agesqr=age*age,
               female=factor(ifelse(sex==0,"Mujer","Hombre")),
               oficio=factor(oficio),
               oficio=relevel(oficio, ref = '99'),
               formal=factor(formal, levels=c(1,0), labels=c("Formal","Informal"))
               )
