...
Así, se busca aquí explorar diferentes especificaciones entre el salario y sus determinantes. Estudiaremos el papel de la edad, la educaicón y del sexo en esta relación, profundizando en la forma como incluir las diferentes ocupaciones/oficios influyen en los resultados [@mincerInvestmentHumanCapital1958]. 
...
Para probar la incidencia de incluir los factores de expansión en los modelos predictivos se realizó un ejercicio en el que se estimó el 
modelo $\log(salaario_i)=\beta_0+\beta_1 Sexo_i + \beta_2 Edad_i + \beta_3 Edad_i^2+u_i$ tres veces: usando el paquete `glm`
con y sin el argumento de pesos `weights`, y usando la función `survey::svyglm`.
