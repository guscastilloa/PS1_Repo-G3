...
stats$exp <- ifelse(stats$exp<0,0,stats$exp)
stats <- data.frame(stats%>%
                      select(c(y_ingLab_m_ha, woman, sector,posicion, exp,
                               age, esc)))
...
gap1 <- lm('log(y_ingLab_m_ha)~age+woman+esc+posicion+sector', data = stats)
